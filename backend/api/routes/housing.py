...
async def get_listings(
    ...
    property_type: Optional[str] = Query(None, description="Type of property (apartment, sublet, room, house)"),
    city: Optional[str] = Query(None, description="City name"),
    state: Optional[str] = Query(None, description="State code"),
    sort_by: str = Query("created_at", description="Sort field (created_at, price, like_count, view_count)"),
...
async def delete_listing(
    ...
    """
    try:
        await housing_service.delete_listing(
            listing_id=listing_id,
            user_id=current_user.id
        )
        return None
...
async def deactivate_listing(
    ...
    """
    try:
        await housing_service.deactivate_listing(
            listing_id=listing_id,
            user_id=current_user.id
        )
...
async def like_listing(
    ...
    """
    try:
        like = await housing_service.like_listing(
            listing_id=listing_id,
            user_id=current_user.id
        )
...
async def unlike_listing(
    ...
    """
    try:
        await housing_service.unlike_listing(
            listing_id=listing_id,
            user_id=current_user.id
        )
        return None
