...
class PostCreate(BaseModel):
    ...
    content: Optional[str] = Field(
        None, 
        max_length=5000, 
        description="Post text content (max 5000 characters)"
    )
    ...
    media_types: Optional[List[str]] = Field(
        default_factory=list,
        description="List of media types ('image' or 'video')"
    )
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        if self.media_urls and self.media_types:
            if len(self.media_urls) != len(self.media_types):
                raise ValueError(
                    f"Number of media_urls ({len(self.media_urls)}) must match media_types ({len(self.media_types)})"
                )
...
class PostUpdate(BaseModel):
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                "content": "Updated: Great day at campus!"
            }
        }
