...
class HousingListingCreate(BaseModel):
    ...
    amenities: List[str] = Field(
        ...
        description="List of amenities (e.g., parking, laundry, pets)"
    )
    ...
    def validate_phone(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            ...
            if not re.match(r'^\+?\d{7,15}$', cleaned):
                raise ValueError("Phone number must contain 7-15 digits (formats accepted: +1-555-0100, (555) 123-4567, 5551234567)")
        return v
...
class HousingListingResponse(BaseModel):
    ...
    is_liked_by_current_user: Optional[bool] = Field(
        None,
        description="Whether current user has liked this listing"
    )
...
class HousingSearchFilters(BaseModel):
    ...
    amenities: List[str] = Field(
        default_factory=list,
        description="Required amenities (listing must have all)"
    )
    available_from: Optional[date] = Field(
        None,
        description="Minimum availability start date"
    )
