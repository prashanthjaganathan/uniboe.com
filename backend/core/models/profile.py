...
class ProfileUpdate(BaseModel):
    ...
    interests: Optional[List[str]] = Field(
        None,
        description="List of user interests (max 20)"
    )
    ...
    def validate_graduation_year(cls, v: Optional[int]) -> Optional[int]:
        ...
        if v < min_year or v > max_year:
            raise ValueError(
                f"Graduation year must be between {min_year} and {max_year}"
            )
...
class ProfileSearchRequest(BaseModel):
    ...
    query: str = Field(
        ...
        min_length=1,
        max_length=100,
        description="Search query (name or university)"
    )
    ...
    interests: Optional[List[str]] = Field(
        None,
        description="Filter by interests (must have at least one)"
    )
    graduation_year: Optional[int] = Field(
        None,
...
class ProfileListResponse(BaseModel):
    ...
    profiles: List[PublicProfileResponse] = Field(
        ...,
...
class ProfileStatsResponse(BaseModel):
    ...
    listings_count: int = Field(
        default=0,
        ge=0,
        description="Number of housing listings created"
    )
    connections_count: int = Field(
        default=0,
        ge=0,
        description="Number of conversations/connections"
    )
