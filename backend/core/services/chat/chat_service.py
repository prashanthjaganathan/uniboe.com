...
from backend.core.utils import encrypt_message, decrypt_message
...
class ChatService:
    ...
    async def get_or_create_conversation(
        ...
        try:
            ...
            existing_response = supabase.table("conversations").select(
                "*, profiles!conversations_participant_1_id_fkey(id, full_name, profile_picture_url, universities(name)),"
                "profiles!conversations_participant_2_id_fkey(id, full_name, profile_picture_url, universities(name))"
            ).eq("participant_1_id", str(p1_id)).eq("participant_2_id", str(p2_id)).execute()
    ...
    async def get_user_conversations(
        ...
        try:
            ...
            query1 = supabase.table("conversations").select(
                "*, profiles!conversations_participant_1_id_fkey(id, full_name, profile_picture_url, universities(name)),"
                "profiles!conversations_participant_2_id_fkey(id, full_name, profile_picture_url, universities(name))"
            ).eq("participant_1_id", str(user_id)).order("last_message_at", desc=True)
            
            query2 = supabase.table("conversations").select(
                "*, profiles!conversations_participant_1_id_fkey(id, full_name, profile_picture_url, universities(name)),"
                "profiles!conversations_participant_2_id_fkey(id, full_name, profile_picture_url, universities(name))"
            ).eq("participant_2_id", str(user_id)).order("last_message_at", desc=True)
    ...
    async def get_conversation_by_id(
        ...
        try:
            ...
            if conv["participant_1_id"] != str(user_id) and conv["participant_2_id"] != str(user_id):
    ...
    async def send_message(
        ...
        try:
            ...
            if conv["participant_1_id"] != str(sender_id) and conv["participant_2_id"] != str(sender_id):
                raise UnauthorizedError("You are not a participant in this conversation")
    ...
    async def mark_conversation_as_read(
        ...
        try:
            ...
            update_response = supabase.table("messages").update({
                ...
                "is_read", False
            ).neq("sender_id", str(user_id)).execute()
    ...
    async def get_unread_count(self, user_id: UUID) -> int:
        ...
        try:
            ...
            conv1 = supabase.table("conversations").select("id").eq(
                "participant_1_id", str(user_id)
            ).execute()
            conv2 = supabase.table("conversations").select("id").eq(
                "participant_2_id", str(user_id)
            ).execute()
            
            conversation_ids = [c["id"] for c in (conv1.data or [])] + [c["id"] for c in (conv2.data or [])]
            ...
            for conv_id in conversation_ids:
                unread_response = supabase.table("messages").select(
                    "id", count="exact"
    ...
    async def delete_conversation(
        ...
        try:
            ...
            delete_response = supabase.table("conversations").delete().eq(
    ...
    def _format_conversation_response(
        ...
        if conv["participant_1_id"] == other_id:
            other_profile = conv.get("profiles", {}) if isinstance(conv.get("profiles"), dict) else {}
        else:
