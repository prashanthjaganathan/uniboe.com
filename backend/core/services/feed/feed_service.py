...
class FeedService:
    ...
    async def get_feed(
        ...
        try:
            ...
            query = supabase.table("posts").select(
                "*, profiles!posts_user_id_fkey(id, full_name, profile_picture_url, university_id, universities(name))",
                count="exact"
            )
    ...
    async def update_post(
        ...
        try:
            ...
            post_check = supabase.table("posts").select("user_id").eq(
                "id", str(post_id)
            ).execute()
            ...
            # Update the post
            response = supabase.table("posts").update(update_dict).eq(
                "id", str(post_id)
            ).execute()
    ...
    async def delete_post(self, post_id: UUID, user_id: UUID) -> bool:
        ...
        try:
            ...
            response = supabase.table("posts").delete().eq(
                "id", str(post_id)
            ).execute()
    ...
    async def like_post(self, post_id: UUID, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            post_check = supabase.table("posts").select("id").eq(
                "id", str(post_id)
            ).execute()
            ...
            existing_like = supabase.table("post_likes").select("*").eq(
                "post_id", str(post_id)
            ...
            if user_info.data and len(user_info.data) > 0:
                ...
                user_dict = {
                    ...
                    "university_name": profile["universities"]["name"] if profile.get("universities") else None
                }
    ...
    async def unlike_post(self, post_id: UUID, user_id: UUID) -> bool:
        ...
        try:
            # Delete the like
            response = supabase.table("post_likes").delete().eq(
                "post_id", str(post_id)
            ).eq("user_id", str(user_id)).execute()
    ...
    async def get_post_likes(
        ...
        try:
            ...
            query = supabase.table("post_likes").select(
                "*, profiles!post_likes_user_id_fkey(id, full_name, profile_picture_url, university_id, universities(name))",
                count="exact"
            ).eq("post_id", str(post_id)).order("created_at", desc=True)
            ...
            if response.data:
                for like_data in response.data:
                    ...
                    user_dict = {
                        ...
                        "profile_picture_url": profile.get("profile_picture_url"),
                        "university_name": profile.get("universities", {}).get("name") if profile.get("universities") else None
                    }
