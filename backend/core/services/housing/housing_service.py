...
class HousingService:
    ...
    async def create_listing(
        ...
        try:
            listing_to_insert = {
                ...
                "available_from": listing_data.available_from.isoformat() if listing_data.available_from else None,
                "available_until": listing_data.available_until.isoformat() if listing_data.available_until else None,
    ...
    async def get_listings(
        ...
        try:
            ...
            query = supabase.table("housing_listings").select(
                "*, profiles!housing_listings_user_id_fkey(id, full_name, profile_picture_url, universities(name))"
            ).eq("is_active", True)
            ...
            for listing_data in listings_response.data:
                ...
                is_liked = False
                if current_user_id:
                    like_check = supabase.table("housing_likes").select("id").eq(
    ...
    async def get_listing_by_id(
        ...
        """
        try:
            response = supabase.table("housing_listings").select(
                "*, profiles!housing_listings_user_id_fkey(id, full_name, profile_picture_url, universities(name))"
            ).eq("id", str(listing_id)).eq("is_active", True).execute()
    ...
    async def update_listing(
        ...
        listing_id: UUID,
        user_id: UUID,
        update_data: HousingListingUpdate
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            existing_listing_response = supabase.table("housing_listings").select(
                "user_id"
            ).eq("id", str(listing_id)).execute()
            ...
            response = supabase.table("housing_listings").update(
                update_payload
            ).eq("id", str(listing_id)).execute()
    ...
    async def like_listing(self, listing_id: UUID, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            listing_check = supabase.table("housing_listings").select("id, is_active").eq(
                "id", str(listing_id)
            ).execute()
            ...
            existing_like_response = supabase.table("housing_likes").select("*").eq(
    ...
    async def unlike_listing(self, listing_id: UUID, user_id: UUID) -> bool:
        ...
        try:
            ...
            response = supabase.table("housing_likes").delete().eq(
                "listing_id", str(listing_id)
            ).eq("user_id", str(user_id)).execute()
    ...
    async def get_listing_likes(
        ...
        Returns:
            A dictionary containing the list of likes with user details, total count, and pagination info.
        ...
        try:
            ...
            listing_check = supabase.table("housing_listings").select("id").eq(
                "id", str(listing_id)
            ).execute()
            ...
            likes_query = supabase.table("housing_likes").select(
                "*, profiles!housing_likes_user_id_fkey(id, full_name, profile_picture_url, universities(name))"
            ).eq("listing_id", str(listing_id)).order("created_at", desc=True)
            ...
            for like_data in likes_response.data:
                ...
                user_info = {
                    ...
                    "university_name": profile_data["universities"]["name"] if profile_data.get("universities") else None
                }
                likes_list.append(HousingLikeResponse(
    ...
    async def search_by_location(
        ...
        try:
            ...
            listings_response = supabase.table("housing_listings").select(
                "*, profiles!housing_listings_user_id_fkey(id, full_name, profile_picture_url, universities(name))"
            ...
            filtered_listings = [
                listing for listing in listings_response.data
                if (query_lower in listing["city"].lower() or
                    query_lower in listing["state"].lower() or
                    query_lower in listing["address"].lower())
            ]
    ...
    def _format_listing_response(
        ...
        return {
            ...
            "created_at": datetime.fromisoformat(listing_data["created_at"]) if isinstance(listing_data["created_at"], str) else listing_data["created_at"],
            "updated_at": datetime.fromisoformat(listing_data["updated_at"]) if isinstance(listing_data["updated_at"], str) else listing_data["updated_at"],
            "user": user_info,
