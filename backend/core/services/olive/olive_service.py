...
class OliveService:
    ...
    def __init__(self):
        ...
        self.default_system_prompt = """You are Olive, an AI assistant for Uniboe - a student life platform.
...
Be helpful, friendly, and concise. Provide accurate information about student rights and regulations.
If you're unsure about specific laws or regulations, advise students to check with official sources.
Keep responses informative but not too lengthy."""
    ...
    async def chat(
        ...
        try:
            ...
            if conversation_id is None:
                ...
                conv_response = supabase.table("olive_conversations").insert({
                    "user_id": str(user_id)
                }).execute()
                ...
                conv_check = supabase.table("olive_conversations").select("*").eq(
            ...
            user_msg_response = supabase.table("olive_messages").insert({
            ...
            history_response = supabase.table("olive_messages").select(
                "role, content"
            ...
            assistant_msg_response = supabase.table("olive_messages").insert({
                "conversation_id": str(conversation_id),
                "role": "assistant",
            ...
            if is_new_conversation:
                ...
                supabase.table("olive_conversations").update({
                    "title": title
    ...
    async def get_conversation(
        ...
        try:
            # Get conversation
            conv_response = supabase.table("olive_conversations").select("*").eq(
                "id", str(conversation_id)
            ).execute()
            ...
            messages_response = supabase.table("olive_messages").select("*").eq(
                "conversation_id", str(conversation_id)
    ...
    async def get_user_conversations(
        ...
        try:
            ...
            # Get conversations
            conv_query = supabase.table("olive_conversations").select(
                "*"
            ...
            for conv in conv_response.data:
                ...
                msg_count_response = supabase.table("olive_messages").select(
                    "id", count="exact"
                ...
                last_msg_response = supabase.table("olive_messages").select(
                    "created_at"
    ...
    async def update_conversation_title(
        ...
        try:
            ...
            # Update title
            update_response = supabase.table("olive_conversations").update({
                "title": title
            }).eq("id", str(conversation_id)).execute()
    ...
    async def delete_conversation(
        ...
        try:
            ...
            delete_response = supabase.table("olive_conversations").delete().eq(
                "id", str(conversation_id)
            ).execute()
