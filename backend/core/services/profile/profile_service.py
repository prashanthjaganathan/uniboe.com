...
class ProfileService:
    ...
    async def get_profile_by_id(
        ...
        try:
            response = supabase.table("profiles").select(
                "*, universities(id, name, domain, state)"
    ...
    async def get_profile_stats(self, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            conv1_response = supabase.table("conversations").select(
                "id", count="exact"
            ).eq("participant_1_id", str(user_id)).execute()
            
            conv2_response = supabase.table("conversations").select(
                "id", count="exact"
            ).eq("participant_2_id", str(user_id)).execute()
            
            connections_count = (
                (conv1_response.count if conv1_response.count else 0) +
    ...
    async def get_user_by_email(self, email: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = supabase.table("profiles").select(
                ...
                "graduation_year, major, university_id, university_email, "
                "is_verified, created_at, universities(id, name, domain, state)"
            ).or_(f"email.eq.{email},university_email.eq.{email}").execute()
    ...
    def _format_profile_response(
        ...
        updated_at = datetime.fromisoformat(profile_data["updated_at"]) if profile_data.get("updated_at") else created_at
