...
class UniversityVerificationService:
    ...
    async def get_university_by_domain(self, domain: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            response = supabase.table("universities").select("*").eq("domain", domain.lower()).execute()
    ...
    async def search_universities(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = supabase.table("universities").select("*").ilike("name", f"%{query}%").execute()
    ...
    async def create_university(self, name: str, domain: str, country: str = "USA", state: Optional[str] = None) -> Dict[str, Any]:
        """
